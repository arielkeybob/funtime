...
## Nota pós-implementação (v2.10.0) — compartilhar pela tela do amigo

Perguntado o que acontece sem evento em andamento ou com eventos desligados (padrão do app): hoje nada se compartilha, porque todo compartilhamento é de um evento (prazo = fim do evento + 24h; a opção só existia no detalhe de um evento já iniciado). Alternativas consideradas: compartilhar "últimas 24h" sem evento, ou criar evento automático. **Decisão do usuário: manter o compartilhamento preso a um evento.** A aba Compartilhando do amigo passou a oferecer: Compartilhar (evento em andamento), Iniciar evento e compartilhar (abre o formulário de novo evento com a pessoa marcada) e, com "Usar eventos" desligado, um aviso com atalho para o interruptor em Configurações. A aba Vendo continua independente dos eventos. `createShareUI` recebe três pontes do app (`getEventsContext`, `startEventWith`, `openEventsSetting`), já que não conhece `state`; `openOccasionEditor(id, shareWith)` ganhou a lista de pessoas pré-marcadas e passou a ser exposta em `globalThis` (occasions-ui.js não é visível como global por si só).
