...
function context() {
  ...
  const c=vm.createContext({state,Date,console,PICKER_ICONS:['🍺','💧'],DATA_STORAGE_KEY:'data',SECURITY_STORAGE_KEY:'security',LEGACY_DRINKS_STORAGE_KEY:'legacy',SECURITY_SESSION_KEY:'session',SHARE_IMPORT_CACHE_NAME:'shared',PIN_LOCKOUT_ATTEMPTS:5,PIN_LOCKOUT_MS:30000,
    document:{querySelector:node,querySelectorAll:()=>[]},localStorage:{getItem:k=>storage.get(k)??null,setItem:(k,v)=>storage.set(k,v),removeItem:k=>storage.delete(k)},sessionStorage:{removeItem(){}},window:{addEventListener(){},caches:{},location:{reload(){c.reloaded=true;}}},caches:{delete:async()=>true},
    buildCurrentAppData:()=>({version:9,drinks:state.drinks,events:state.events,preferences:state.preferences}),hideToast(){},applyInterfacePreferences(){},refreshDataViews(){},updateDataSettingsUI(){},showToast(){},showAppNotification(message,options){c.notification={message,...options};},getDefaultSecurityConfig:()=>({enabled:false,method:null}),clearTimeout(){},getConfiguredPinLength:()=>4,normalizePinInput:i=>i.value,getPinLockoutRemainingMs:()=>Math.max(0,state.pinLockoutUntil-Date.now()),verifyPin:async()=>true,verifyDeviceCredential:async()=>true,registerFailedPinAttempt:()=>{state.pinFailedAttempts+=1;const lockedOut=state.pinFailedAttempts>=5;if(lockedOut)state.pinLockoutUntil=Date.now()+30000;return lockedOut;}});
...
test('apagar tudo limpa dados e proteção sem apagar aceite ou outras aplicações',async()=>{
 ...
 assert.equal(c.storage.has('legacy'),false);assert.equal(c.storage.has('security'),false);assert.equal(c.storage.get('intervalo-terms-v1'),'accepted');assert.equal(c.storage.get('other-app'),'keep');assert.equal(c.reloaded,undefined);assert.equal(c.notification.persistent,true);c.notification.onDismiss();assert.equal(c.reloaded,true);
});
